...
namespace ThemeTwister {
    public class TwisterPlasma : Gtk.Box {
        public TwisterPlasma () {
            var wrapper = new Gtk.Box (Gtk.Orientation.VERTICAL, 0);
            wrapper.get_style_context ().add_class ("theme");

	        var pixbuf = new Gdk.Pixbuf.from_file("/usr/local/share/themetwister/TwisterPlasma.png");
			...
			button.clicked.connect (() => {
		        try {
		        	GLib.AppInfo info = AppInfo.create_from_commandline("xfce4-terminal --title=Config --hide-menubar --hide-borders --hide-scrollbar -e \"/usr/share/ThemeSwitcher/ThemeTwister TwisterPlasma\"", null, AppInfoCreateFlags.SUPPORTS_STARTUP_NOTIFICATION);
			...
			button.clicked.connect (() => {
		        try {
		        	GLib.AppInfo info = AppInfo.create_from_commandline("xfce4-terminal --title=Config --hide-menubar --hide-borders --hide-scrollbar -e \"/usr/share/ThemeSwitcher/ThemeTwister TwisterPlasma-Dark\"", null, AppInfoCreateFlags.SUPPORTS_STARTUP_NOTIFICATION);
		        	info.launch(null,Gdk.Display.get_default().get_app_launch_context());
		        } catch (GLib.Error e){warning ("Could not load Config: %s", e.message);}
			});
	        ...
	        var description = new Label ("Twister Plasma");
